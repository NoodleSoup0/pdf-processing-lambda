...
def lambda_handler(event, context):
  try:
    ...
    for i in range(10):
      outfile.write(str(i))
      outfile.write(" " + str(count_array[i]) + "\n")
    '''
    outfile.write("Detected Language: " + language)
    ...
    stop_words = set(['the', 'and', 'in', 'of', 'to', 'a', 'is', 'for', 'with', 'on', 'this', 'it'])

    # Filter out stop words and numeric words
    ...
    print("** Keywords for the Entire Document **")
    print(top_keywords)
   #########################################################################
    #
   ...
   # Loop through the keyword IDs
    for keyword, frequency in top_keywords:
      print("keyword", keyword)
      # Insert the keyword into the database
      sql_insert_keyword = "INSERT IGNORE INTO keywords (keyword) VALUES (%s);"
      datatier.perform_action(dbConn, sql_insert_keyword, [keyword])
  
      # # Perform lookup to get the keyword ID
      # sql_lookup_keyword_id = "SELECT keywordid FROM keywords WHERE keyword = %s;"
      # keyword_id = datatier.retrieve_one_row(dbConn, sql_lookup_keyword_id, [keyword])
      # print("keyword_id:",keyword_id[0])
  
      
  
    print("Show keyword table")
